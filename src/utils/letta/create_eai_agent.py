...
def _merge_config(base_config: dict, override_payload: dict | None) -> dict:
    ...
    with tracer.start_as_current_span("create_eai_agent.merge_config") as span:
        span.set_attribute("create_eai_agent.has_override", override_payload is not None)
        if override_payload:
            span.set_attribute("create_eai_agent.override_keys", list(override_payload.keys()))
...
async def create_eai_agent(user_number: str, override_payload: dict | None = None):
    with tracer.start_as_current_span("create_eai_agent.create") as span:
        ...
        span.set_attribute("create_eai_agent.has_override_payload", override_payload is not None)
        ...
        try:
            ...
            span.set_attribute("create_eai_agent.system_prompt_length", len(system_prompt))
            
            # Get agent config
            agent_config = await get_agent_config_from_api(agent_type="agentic_search")
            span.set_attribute("create_eai_agent.config_keys", list(agent_config.keys()))
