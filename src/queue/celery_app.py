...
from src.config.telemetry import instrument_celery, setup_telemetry

# Setup OpenTelemetry before creating Celery app
setup_telemetry()

celery = Celery("gateway", broker=env.REDIS_DSN, backend=env.REDIS_BACKEND)
...
celery.conf.update(
    ...
    if env.CELERY_WORKER_POOL == "prefork"
    else None,
)
