...
def process_user_message(self, message_id: str, user_number: str, message: str, previous_message: str | None = None) -> None:
    ...
    with tracer.start_as_current_span("celery.process_user_message") as span:
        ...
        try:
            logger.info(f"[{self.request.id}] Processing user message {message_id} for user {user_number}")
            ...
            if agent_id is None:
                ...
                logger.info(f"[{self.request.id}] Creating new agent for user {user_number}")
                agent_id = letta_service.create_agent_sync(user_number=user_number)
            else:
                span.set_attribute("celery.agent_found", True)
                logger.info(f"[{self.request.id}] Found existing agent {agent_id} for user {user_number}")
            ...
            logger.info(f"[{self.request.id}] Successfully processed user message {message_id} for agent {agent_id}")
            ...
            logger.warning(f"[{self.request.id}] Soft time limit exceeded for user message {message_id}: {exc}")
            ...
            logger.warning(f"[{self.request.id}] Letta API timeout for user message {message_id}: {exc}")
            if self.request.retries >= self.max_retries:
                span.set_attribute("celery.max_retries_exceeded", True)
                logger.error(f"[{self.request.id}] Max retries exceeded for user message {message_id}")
                store_response_sync(message_id, {
                    "status": "error",
            ...
            logger.warning(f"[{self.request.id}] Letta API error for user message {message_id}: {exc}")
            if self.request.retries >= self.max_retries:
                span.set_attribute("celery.max_retries_exceeded", True)
                logger.error(f"[{self.request.id}] Max retries exceeded for user message {message_id}")
            ...
            logger.warning(f"[{self.request.id}] Retryable HTTP error for user message {message_id}: {exc}")
            if self.request.retries >= self.max_retries:
                span.set_attribute("celery.max_retries_exceeded", True)
                logger.error(f"[{self.request.id}] Max retries exceeded for user message {message_id}")
                store_response_sync(message_id, {
            ...
            logger.error(f"[{self.request.id}] Fatal error processing user message {message_id}: {exc}")
