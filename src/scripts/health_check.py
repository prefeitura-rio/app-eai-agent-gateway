...
async def run_health_check():
    ...
    if not letta_ok:
        ...
        logger.info(
            "  - Verifique as variáveis LETTA_API_URL e LETTA_API_TOKEN no .env",
        )
        logger.info(
            "  - Teste manualmente: curl -H 'Authorization: Bearer TOKEN' URL/health",
        )
    ...
    if not workers_ok:
        ...
        logger.info(
            "  - Ou manualmente: uv run celery -A src.queue.celery_app.celery worker --loglevel=info",
        )
